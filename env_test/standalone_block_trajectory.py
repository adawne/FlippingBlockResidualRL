...
def main(render_modes, randomize_params):
    ...
    desired_angle_quat_array = R.from_euler('xyz', [0, np.pi-2.0945, -3.14]).as_quat()
    ...
    print(f"Lentgh qpos: {len(data.qpos.copy())}")
    ...
    while data.time < 3:
        ...
        if flipped:
            ...
            #if block_orientation_euler_deg[1] > -60:
                #if prev_orientation_y is not None and block_orientation_euler_deg[1] > prev_orientation_y:
                #if prev_orientation_y is not None and quat_distance(block_orientation, desired_angle_quat_array) > 0.99:
            ...
            if block_trans_velocity[0] < v_x_desired:
                data.xfrc_applied[block_body_id][0] = 0.05

            else:
                data.xfrc_applied[block_body_id][0] = 0

            if block_trans_velocity[2] < v_z_desired:
                data.xfrc_applied[block_body_id][2] = 0.2
            else:
