...
def main(iteration, render_modes, contact_vis, random_mass, block_mass, block_size, ee_flip_target_velocity, sim_description, 
        ...
        pad_solimp, pad_solref, clampness, use_combinations):
    ...
    for i in range(iteration):
        ...
        current_config = {'iteration': i, 'render_modes': render_modes, 'contact_vis': contact_vis,
                        'random_mass': random_mass,'block_mass': block_mass,'block_size': block_size,
                        'ee_flip_target_velocity': ee_flip_target_velocity, 'sim_description': sim_description,
                        'block_solimp': block_solimp, 'block_solref': block_solref, 'block_friction': block_friction,
                        'cone': cone, 'noslip_iterations': noslip_iterations, 'noslip_tolerance': noslip_tolerance,
                        'impratio': impratio, 'pad_friction': pad_friction, 'pad_solimp': pad_solimp,'pad_solref': pad_solref,
                        'clampness': clampness, 'use_combinations': use_combinations}
        ...
        world_xml_model = create_ur_model(marker_position=None, block_positions_orientations=block_positions_orientations, 
                                        block_mass=block_mass, block_size=block_size, block_solimp=block_solimp, block_solref=block_solref, 
                                        block_friction=block_friction, cone=cone, noslip_iterations=noslip_iterations, noslip_tolerance=noslip_tolerance,
                                        impratio=impratio, pad_friction=pad_friction, pad_solimp=pad_solimp, pad_solref=pad_solref)
        ...
        while has_block_steady == False and data.time < 8:
            ...
            # Flip block
                ...
                if fsm.has_block_flipped == False:
                    ...
                    #print(np.linalg.norm(block_trans_velocity), np.linalg.norm(block_ang_velocity))
        ...
        if use_combinations is not True:
            ...
            time_discrepancy_percentage, angle_discrepancy_percentage, height_discrepancy_percentage, landing_velocity_discrepancy_percentage = check_physical_assumptions(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_combinations', action='store_true', help="Use parameter combinations")
    
    args = parser.parse_args()
    
    if args.use_combinations:
