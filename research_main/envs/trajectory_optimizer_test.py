...
def objective(x, I, m, lambda_1, lambda_2):
    ...
    potential_energy_penalty = lambda_1 * m * v_y0**2 + lambda_2 * m * g * h_release
    return rotational_energy + potential_energy_penalty

def objective_with_regularization(x, I, m, lambda_1, lambda_2):
    regularization = 1e-6 * np.sum(x**2)
    return objective(x, I, m, lambda_1, lambda_2) + regularization
...
bnds = [(0.1, None), (0.1, None), (0, 2*np.pi/3), (0, None)]
...
lambda_1 = 0.1  # Weighting factor for vertical velocity
lambda_2 = 0.1  # Weighting factor for release height
...
for _ in range(4):
    ...
    for x0 in x0_list:
        result = minimize(objective_with_regularization, x0, args=(I, m, lambda_1, lambda_2), method='trust-constr', bounds=bnds, constraints=constr)
